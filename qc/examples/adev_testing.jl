...
function f(theta)
    @adev begin
        let x = @sample(flip_mvd(theta[1])) & @sample(flip_mvd(theta[1])) & !@sample(flip_mvd(theta[1])),
            y = @sample(flip_mvd(theta[2])) & !@sample(flip_mvd(theta[2])) & !@sample(flip_mvd(theta[2]))
            x & y
        end
    end
end
...
function fab(a, b)
    @adev begin
        let x = @sample(fl(a)) & @sample(fl(a)) & !@sample(fl(a)),
            y = @sample(fl(b)) & !@sample(fl(b)) & !@sample(fl(b))
            x & y
        end
    end
end
gradient(fab; a=0.5, b=0.5)

ADEV.gradient(fab, 0.5, 0.5)


ct = 0
for seed in 1:1000
    global ct
    res = ReverseDiff.gradient( (args...) ->
            fab([arg[1] for arg in args]...)(r -> (wants_grad, rng) -> r)(true, Random.Xoshiro(seed)),
        ([0.5], [0.5])
    )
    if res != ([0.0], [0.0])
        @show seed
...
ReverseDiff.gradient(
    (args...) ->
        fab([arg[1] for arg in args]...)(r -> (wants_grad, rng) -> r)(true, Random.Xoshiro(3)),
    ([0.5], [0.5])
)


sgd(f; a=0.5, b=0.5, step_size=0.01, n_iters=1000, N=100)
