export DiceProgram, Flip, Categorical, DiceTuple, Identifier, EqualsOp, Ite, LetExpr

abstract type DiceExpr end
struct DiceProgram
    expr::DiceExpr
end

struct Flip <: DiceExpr
    prob::Float64
end
struct Categorical <: DiceExpr
    probs::Vector{Float64}    
end
struct DiceTuple <: DiceExpr
    # avoid too many nested type parameters by not using Tuple here
    first::DiceExpr
    second::DiceExpr
end
struct DiceBool <: DiceExpr
    b::Bool
end
struct DiceInt <: DiceExpr
    v::Int
    num_bits::Int
end
struct Identifier <: DiceExpr
    symbol::String    
end

struct EqualsOp <: DiceExpr
    e1::DiceExpr
    e2::DiceExpr
end

struct Ite <: DiceExpr
    cond_expr::DiceExpr
    then_expr::DiceExpr
    else_expr::DiceExpr
end

struct LetExpr <: DiceExpr
    identifier::Identifier
    e1::DiceExpr
    e2::DiceExpr
end

num_bits(i::DiceInt) = i.num_bits
num_bits(i::Int) = ceil(Int,log2(i+1))
num_bits(x::Ite) = 
    max(num_bits(x.then_expr),num_bits(x.else_expr))
    
function num_bits(x::Categorical)
    max_val = findlast(p -> !iszero(p), x.probs) - 1
    num_bits(max_val)
end

# TODO add other cases for num_bits?

Base.show(io::IO, x::DiceProgram) =
    show(io, x.expr, 0) 

Base.show(io, e::DiceInt, ident) =
    print(io, "int($(e.num_bits),$(e.v))") 

Base.show(io, e::DiceBool, ident) =
    print(io, "$(e.b)") 

function Base.show(io::IO, e::LetExpr, ident = 0)
    println(io, "let $(e.identifier) = ")
    print_indentation(io, ident + 1)    
    show(io, e.e1, ident + 1)
    println(io, " in")
    print_indentation(io, ident)    
    show(io, e.e2, ident) 
end

function Base.show(io::IO, e::Categorical, ident = 0)
    print(io, "discrete(")
    join(io, e.probs, ", ")
    print(io, ")") 
end

function Base.show(io::IO, e::Flip, ident = 0)
    print(io, "flip $(e.prob)")
end

function Base.show(io::IO, e::Ite, ident = 0)
    print(io, "if ")
    show(io, e.cond_expr, ident)
    println(io, " then")
    print_indentation(io, ident+1)    
    show(io, e.then_expr, ident + 1)
    println(io)
    print_indentation(io, ident)    
    println(io, "else")
    print_indentation(io, ident+1)    
    show(io, e.else_expr, ident + 1) 
end

function Base.show(io::IO, e::DiceTuple, ident = 0)
    print(io, "(") 
    show(io, e.first, ident)
    print(io, ", ") 
    show(io, e.second, ident)
    print(io, ")") 
end

function Base.show(io::IO, e::EqualsOp, ident = 0)
    show(io, e.e1, ident)
    print(io, " == ") 
    show(io, e.e2, ident)
end

function Base.show(io::IO, e::Identifier, ident = 0)
    print(io, e.symbol) 
end

print_indentation(io, ident) =
    print(io, "  "^ident)

