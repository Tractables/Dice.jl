...
for bits in 1:20
    _, _, bdd_size = spikenslab_collapsed_sample(bits, bits + 2, n, p, X, y, 10)
    io = open("examples/sample/spikenslab/spikenslab_scale_with_l.txt", "a")
    @show  [bits+4 bits+2 n p s bdd_size]
    writedlm(io, [bits+4 bits+2 n p s bdd_size], ",")
    close(io)
end
