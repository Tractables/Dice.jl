...
# LINEAR = true  # by default, the desired distribution is uniform. 

for (INIT_SIZE, GEN_TYP_SIZE) in [(3, 1), (4, 1), (3, 2), (4, 2)]
    for METRIC in [num_apps, term_size]
        for LINEAR in [false, true]
            for PARAMETERIZE_FLIP_GROUPS_BY_SZ in [false, true]
                reset_flips!()

                ############################

                dist_desc = if LINEAR "linear" else "uniform" end
                OUT_DIR = "examples/qc/stlc/output2/$(METRIC)/$(dist_desc)/sz=$(INIT_SIZE),tysz=$(GEN_TYP_SIZE)"
