...
function expand_symbol(sym, max_depth)
    if sym isa Terminals.Terminals_
        DistVector(DistEnum[DistEnum(sym)]), false # Terminal, no error
    elseif max_depth == 0
        DistVector{DistEnum}(), true  # Unexpandable nonterminal, error
    else
        # Sample from expansion of rules
        # TODO: use this instead once we allow dynamo to transform iterate()
        # discrete((expand_rhs(rhs, max_depth), p) for (rhs, p) in rules[sym])
        v = []
        for (rhs, p) in rules[sym]
            x = expand_rhs(rhs, max_depth)
            push!(v, (x, p))
        end
        discrete(v)
    end
