...
function gauss_mix_asym_prior(p::Int, binbits::Int)
    code = @dice begin
        ...
        mu1 = add_bits(continuous(dicecontext(), p, t, Normal(2.75, 0.5), 0), binbits + 4, 0)
        mu2 = add_bits(continuous(dicecontext(), p, t, Normal(-2.75, 0.5), 0), binbits + 4, 0)
        ...
        for i = 1:1
            ...
            g1 = add_bits(continuous(dicecontext(), p, t_2, Normal(0, 1), 0), binbits + 4, 0)
            temp1 = (t_res((g1.number * sigma1.number)[1].bits[3:2*binbits + 10]) + mu1)[1]

            #adding precision 2, adding bits (binbits + 4, 0)
            g2 = add_bits(continuous(dicecontext(), p, t_2, Normal(0, 1), 0), binbits + 4, 0)
